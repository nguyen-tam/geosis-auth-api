...
module.exports = {
    ...
    ssoPostLogin: async (ctx, user, serviceURL = null) => {
        ...
        const ssoToken = await strapi.plugins['users-permissions'].services.jwt.issue(_.pick(user.toJSON ? user.toJSON() : user, ['_id', 'id']));
        ...
        const EXPIRE_TIME_TOTAL =  await strapi.plugins['single-sign-on'].config.timeToExpire; //60 - set expire time = 60s
        ...
        if(ctx.request.query && ctx.request.query.serviceURL != undefined){
          serviceURL  = ctx.request.query.serviceURL;
            ...
            await strapi.plugins['single-sign-on'].services.singlesignon.storeApplicationInCache(url.origin, user.id, ssoToken);
    ...
    storeApplicationInCache: async (origin, id, intrmToken) => {
        ...
        if (sessionApp[id] == null) {
            ...
            await strapi.plugins['single-sign-on'].services.singlesignon.fillIntrmTokenCache(origin, id, intrmToken);
        } else{
            let value = {...sessionApp};
            // let value = {};
            value[id][originAppName[origin]] =  true;
